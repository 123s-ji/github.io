var config = {
    ...
    welcome: "你好，我是纪舒，一名拥有6年丰富经验的软件测试工程师。<br>" +
    ...
    */  
    ...
    portfolio: [
        ...
        "2024.3-至今",
        "<h5>项目介绍:</h5>" +
        "matrix自动化测试平台是镁佳公司自主研发得车机自动化测试平台，已与东风、奇瑞、江铃、长安等车企达成合作提供相关技术支持，完成自动化测试计划。<br>" +
        "平台采用 pytest 框架执行用，   定位方式选用 uiautomator2,  can信号工具采用：cantools <br>" +
        "使用PO设计模式，创建各项目得UI元素以供选择使用，定位方式有xpath，selector供使用人员选择<br>" +
        "每一条测试用例完成后，被调用时会根据用例ID，获取脚本。生成测试函数，使用pytest参数化技术，达到用例得筛选，与次数限制。" ,
            [
                "完成签约客户得自动化实施计划，编写压力测试脚本，功能测试脚本，按定制计划执行。分析维护任务单，同步缺陷给甲方，输出相关log数据以供开发定位修改",
                "完成广汽埃安AH8系统级压测用例编写",
                "完成江铃福特756/743压测用例实施，空调、车辆设置、语音模块得功能用例编写",
                "完成奇瑞实车压测计划实施",
                "平台中控0代码功能内部测试，培训相关人员达到公司目的",
                "同步测试平台产出bug到相关jira平台，获取相关日志并上传",
                "编写偶现BUG脚本，复现bug,提供相关日志",
                "汇报工作情况包括遇到的困难、如何解决以及解决的问题等"
            ]
        ],
    ...
    work: [
        ["2024.3 — 至今", "<br>自动化实施工程师",
            ...
            "<p>编写压力测试脚本，功能测试脚本，按定制计划执行，分析维护任务单；</p>" +
            "<p>广汽埃安AH8系统级压测用例编写；</p>"+
            "<p>江铃福特756/743压测用例实施，空调、车辆设置、语音模块得功能用例编写；</p>"+
            "<p>奇瑞实车压测用例编写；</p>"+
            "<p>中控0代码平台内部功能测试；</p>"+
            "<p>实车盒子自动化测试模块功能测试，奇瑞实车压测计划用例维护，任务单分析</p>"+
            "<p>机械臂测试台架功能验证与维护；</p>"
        ],
